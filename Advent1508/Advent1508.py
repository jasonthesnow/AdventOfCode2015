...
puzzle_raw_input = puzzle_file.readlines()

puzzle_modified_input = [
    "qxfcsmh",
...
for i in range(len(puzzle_raw_input)):
    raw_length += len(puzzle_raw_input[i].strip())
    print(f'{puzzle_raw_input[i].strip()} is {raw_length} long')
    
    modified_length += len(puzzle_modified_input[i])
    print(f'{puzzle_modified_input[i]} is {modified_length} long')

print(f'Raw Lenght is: {raw_length}. Modified Length is: {modified_length}. Difference between the two is {raw_length - modified_length}')
